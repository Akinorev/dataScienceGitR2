...
```{r MuestreoEstratificado}
table(mas$NAME_EDUCATION_TYPE)
prop.table(table(mas$NAME_EDUCATION_TYPE))

sum(is.na(mas$NAME_EDUCATION_TYPE))
#the method is 'srswor' (equal probability, without replacement)
strata.base=strata(mas,c("NAME_EDUCATION_TYPE"),size=c(10,5,5,5,2), method="srswor",TRUE)
strata.base
```
...
#importe solicitado del cliente Media Poblacion
media.importe=mean(mas$AMT_CREDIT)
dt.importe=sd(mas$AMT_CREDIT)
media.importe
dt.importe
```
...
```{r MediaPoblacionalIngresos}
#ingresos del cliente Media Poblacion
media.ingresos=mean(mas$AMT_INCOME_TOTAL)
media.ingresos
```

Media muestral de los ingresos del cliente
```{r MediaMuestralIngresos}
...
```{r EdadMediaPoblacional}
#edad media poblacional 
dias=(mas$DAYS_BIRTH*-1)/365.25
...
```{r CalculosImportesClientesPago}
...
```{r CalculosImportesClientesImpago}
...
```{r CalculosAntiguedadClientesPago}
...
```{r CalculosAntiguedadClientesImpago}
...
```{r AnalisisImporteMedioPoblacion}
...
```{r AnalisisAntiguedadEmpresaPoblacion}
...
```{r ContrasteHipotesisPagoImpagoEducacion}

#### test chi2
#### Ho : Nivel de Estudios e Impago de Crédito son Independientes
#### H1 : existe dependencia

chisq.test(mas$TARGET,mas$NAME_EDUCATION_TYPE,correct=FALSE )
chisq.test(mas$TARGET,mas$NAME_EDUCATION_TYPE)$expected
mosaicplot(mas$TARGET,mas$NAME_EDUCATION_TYPE, color=TRUE, main="Plot de mosaico")
...
uno<-table (mas$NAME_EDUCATION_TYPE,mas$TARGET)
prop.table(uno,1)
