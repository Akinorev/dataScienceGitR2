...
### Validacion del modelo Logit con el dataset de Validacion
```{r validation on test for Logit}


predictionsLogit <- predict(model_logit1, control,type='response')
plot.roc(control$target, predictionsLogit)
table(pred = predictionsLogit > 0.5, obs = control$target)
data = as.numeric(predictionsLogit>0.5)
data=as.factor(data)
y_test=as.factor(control$target)
    # use caret and compute a confusion matrix
confusionMatrix(data, y_test)
...
### Validacion del modelo Probit con el dataset de Validacion
```{r validation on test Probit}


predictionsProbit <- predict(model_probit1, control,type='response')
plot.roc(control$target, predictionsProbit)
table(pred = predictionsProbit > 0.5, obs = control$target)
...
Vamos a calcular la precisión general o tasa de error para un Cutoff  de 0.5 obtenemos un 12,4% de tasa de error

```{r funcion para calcular el Error Rate}
