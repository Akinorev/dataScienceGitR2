...
```{r Cluster Jerarquico primera parte dentro del TSNE2 }
...
```
  ...
  Finalmente seleccionamos 3 Grandes Clusters para continuar con los análisis.



```{r Cluster nuevo Cluster a partir del jerarquico anterior TSNE2 }


prueba$hclust<-as.integer(prueba$hclust)
prueba$cluster2 [prueba$hclust==2|prueba$hclust==9] <- "Top"
prueba$cluster2 [prueba$hclust==3|prueba$hclust==8] <- "Med"
prueba$cluster2 [prueba$hclust==4|prueba$hclust==6] <- "Med"
prueba$cluster2 [prueba$hclust==5|prueba$hclust==7|prueba$hclust==1] <- "Low"


ftable(prueba$cluster2)
...
prueba %>%
        group_by(cluster2) %>%
        summarise(num_casas=n(), precio_medio= mean(price), room=mean(bedrooms),baths=mean(bathrooms) ,
                  tamanyo=mean(sqft_living), anyo=mean(yr_built) ,grade=mean(grade), condicion=mean(condition), visitas=mean(view))


prueba$cluster2<-as.factor(prueba$cluster2)
ggplot(data=prueba, aes(x=price, group=cluster2 ,fill=cluster2)) +
...
## Seleción Final de Clusters
  


```{r tsne representación de las variables dentro del tsne2 }
